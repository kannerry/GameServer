...
namespace Passives
{
    public class MasterYiPassive : ICharScript
    {
        ...
        public void TargetExecute(IAttackableUnit unit, bool arg2)
        {
            ...
            if (i == 4)
            {
                var targ = _owner.TargetUnit;
                _owner.RemoveBuffsWithName("DoubleStrikeStacks");
                CreateTimer(0.15f, () => { if(!(targ.IsDead)) { SpellCast(_owner, 0, SpellSlotType.ExtraSlots, _owner.TargetUnit.Position, _owner.TargetUnit.Position, false, Vector2.Zero); }; });
                i = 0;
            }
        }
