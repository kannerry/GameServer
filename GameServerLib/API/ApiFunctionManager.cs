...
namespace LeagueSandbox.GameServer.API
{
    ...
    public static class ApiFunctionManager
    {
        ...
        public static IChampion AddChampion(TeamId team, int userId, string model, Vector2 pos)
        {
            var _playerManager = _game.PlayerManager;
            var championPos = pos;
            ...
            _game.PacketNotifier.NotifyS2C_CreateHero(clientInfoTemp);
            _game.PacketNotifier.NotifyAvatarInfo(clientInfoTemp);
            _game.ObjectManager.AddObject(c);
            _game.PacketNotifier.NotifyEnterLocalVisibilityClient(c, ignoreVision: true);
            _game.PacketNotifier.NotifyUpdatedStats(c, false);
